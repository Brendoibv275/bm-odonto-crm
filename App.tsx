...
// Função utilitária para sanitizar objetos para o Firestore
const sanitizeForFirestore = (obj: any) => {
  return JSON.parse(JSON.stringify(obj, (key, value) => {
    if (value === undefined) {
      return null; // Converte undefined para null
    }
    return value;
  }));
};
...
const App: React.FC = () => {
  ...
  const handleSavePaciente = useCallback(async (pacienteData: Omit<Paciente, 'id' | 'odontograma' | 'anamnese' | 'exameExtraoral' | 'exameIntraoral'> | Paciente) => {
    ...
    try {
      if ('id' in pacienteData && pacienteData.id) { // Editar
        ...
        const { id, ...dataFromForm } = pacienteData;
        
        // Sanitiza o objeto antes de enviar para o Firestore
        const dataToUpdate = sanitizeForFirestore(dataFromForm);
        ...
        const novoPacienteBase: Omit<Paciente, 'id'> = {
          ...
          anamnese: null, // Inicializa como null
          exameExtraoral: null, // Inicializa como null
          exameIntraoral: null, // Inicializa como null
        };
        
        // Sanitiza o objeto antes de enviar para o Firestore
        const novoPacienteParaFirestore = sanitizeForFirestore(novoPacienteBase);
  ...
  const handleSaveAnamnese = useCallback(async (pacienteId: string, anamneseData: AnamneseClinicaData) => {
    ...
    try {
      const cleanAnamneseData = sanitizeForFirestore(anamneseData);
  ...
  const handleSaveExameExtraoral = useCallback(async (pacienteId: string, exameData: ExameExtraoralData) => {
    ...
    const pacienteRef = doc(firebaseDB, collectionPath, pacienteId);
    try {
      const cleanExameData = sanitizeForFirestore(exameData);
  ...
  const handleUpdatePacienteOdontograma = useCallback(async (pacienteId: string, updatedTeeth: Tooth[]) => {
    ...
    const pacienteRef = doc(firebaseDB, collectionPath, pacienteId);
    try {
      const serializableTeeth = sanitizeForFirestore(updatedTeeth);
  ...
  const handleAddAgendaEvent = useCallback(async (eventData: Omit<AgendaEvent, 'id'>) => {
    ...
    if (!collectionPath) return;
    try {
      const dataToSave = sanitizeForFirestore(eventData);
  ...
  const handleUpdateAgendaEvent = useCallback(async (updatedEvent: AgendaEvent) => {
    ...
    const dataToUpdate = sanitizeForFirestore(data);
    try {
  ...
  const handleAddTransacaoFinanceira = useCallback(async (transacaoData: Omit<TransacaoFinanceira, 'id'>) => {
    ...
    if (!collectionPath) return;
    try {
      const dataToSave = sanitizeForFirestore(transacaoData);
      await addDoc(collection(firebaseDB, collectionPath), dataToSave);
